            ...
            {data?.map((item: typeof data[0]) => {
              ...
              return (
                <tr key={item.id}>
                  ...
                  <td className="whitespace-nowrap py-4 pl-3 pr-4 text-sm font-medium sm:pr-6">
                    <Link to={`/leader/referral/${item?.referral?.id}`} className="text-blue-600 hover:text-blue-900">
                      {' '}
                      Edit
                      <span className="sr-only">
                        ,{' '}
        ...
        >
          <div className="hidden sm:block">
            <p className="text-sm text-gray-700">
              ...
              <span className="font-medium">{pagination?.counters?.count}</span> results with a total transaction amount
              of <span className="font-medium">${pagination?.counters?.sum}</span>
            </p>
          </div>
