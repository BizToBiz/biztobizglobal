...
@Injectable()
export class ApiTestimonialDataAccessLeaderService {
  ...
  leaderTestimonials(info: GraphQLResolveInfo, leaderId: string, input?: AdminListTestimonialInput) {
  ...
  async leaderCountTestimonials(leaderId: string, input?: AdminListTestimonialInput): Promise<CorePaging> {
  ...
  leaderTestimonial(info: GraphQLResolveInfo, leaderId: string, testimonialId) {
    const select = new PrismaSelect(info).value
    return this.data.testimonial.findUnique({ where: { id: testimonialId }, ...select })
  }

  leaderCreateTestimonial(info: GraphQLResolveInfo, leaderId: string, input: AdminCreateTestimonialInput) {
    const select = new PrismaSelect(info).value
  ...
  leaderDeleteTestimonial(info: GraphQLResolveInfo, leaderId: string, testimonialId) {
